...
sar_panel_real <- sar_panel |>
    mutate(
        ...
        baseline_apuc_total_2024 = baseline_apuc_total * deflator,
        current_apuc_total_2024 = current_apuc_total * deflator,
...
group_summary <- function(data) {
    tibble(
        ...
        current_apuc_total_2024 = mean(data$current_apuc_total_2024, na.rm = TRUE),
        pauc_growth = mean(data$pauc_growth, na.rm = TRUE),
        apuc_growth = mean(data$apuc_growth, na.rm = TRUE)) |>
        bind_cols(
            data |>
...
summary_stats <- sar_panel_real |>
    mutate(
        ...
        apuc_growth = apuc_growth * 100) |>
    summarise(
        across(
            c(baseline_pauc_total_2024, baseline_apuc_total_2024),
